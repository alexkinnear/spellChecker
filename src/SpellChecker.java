...
//         Make sure it displays in sorted order
    ...
    public static BinarySearchTree<String> readDictionary() {
        ...
        try {
            ...
            java.util.Collections.shuffle(myList, new java.util.Random(System.currentTimeMillis()));
            for (int i = 0; i < myList.size(); i++) {
                tree.insert(myList.get(i));
            }
        }
        catch(Exception ex) {
        ...
        try {
            File spellCheck = new File("Letter.txt");
            java.util.Scanner input = new java.util.Scanner(spellCheck);
            while(input.hasNextLine()) {
            ...
            System.out.println("An error occured in reading from the file");
        }
